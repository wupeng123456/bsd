...
const App = ({
  ...
  const siderProps = {
    ...
    tabListRoute (item) {
      ...
      if (tabList.filter((index) => { return (index.name === item.key)}).length === 0) {
        tabList.push(item)
        const removal = Array.from(new Set(tabList))
        dispatch({ type: 'app/addtabList', payload: { tabList: removal } })
      }
    },
  }
  ...
  const breadProps = {
    ...
    children,
    remove (item) {
      dispatch({ type: 'app/removeList', payload: { tabList: item }})
    },
  }
